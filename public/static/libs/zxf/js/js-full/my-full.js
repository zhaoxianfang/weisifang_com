...
var my = {
    ...
    form: function() {
        ...
        if (arguments.length > 0) {
            ...
            var form = $("form").not(".form-commonsearch, .unbind-form");
        }
        $(form).unbind();
        if (form) {
    ...
    formSubmit: function(form, e) {
        ...
        my.ajax(form_url, formdata, function(succ) {
            ...
            if (succ.code == 200) {
                layer.msg(succ.msg );
                window.toastr.success(succ.msg )
            } else {
                //内容,标题
                if (succ.code == '500' && !succ.msg) {
                    layer.msg('系统异常:请与管理员联系');
                    window.toastr.error('系统异常:请与管理员联系',"出错啦")
                } else {
                    window.toastr.error(succ.msg,"出错啦")
                    layer.msg(succ.msg);
                }
            }
            ...
            layer.msg(errorRes);
            window.toastr.error(errorRes,'出错啦')
            try {
            ...
            if (typeof(form_after) == "function") {
                // form_after(jqXHR.responseText);
                if (form_after(errorRes) === false) {
                    e && e.preventDefault();
                    return false;
                }
            }
    ...
    getFormData: function(form) {
        ...
        $(form).find(':file').each(function(i, v) {
            if( my.isEmpty( $(v).attr('name') ) ===  false  ){
                upFile = true;
                formDataObj.append($(v).attr('name'), $(v)[0].files[0]);
            }
        });
        if (upFile) {
    ...
    isEmpty: function(obj) {
        if (typeof obj == "undefined" || obj == null || obj == "" || obj.length == 0 || (!/[^(^\s*)|(\s*$)]/.test(obj))
            || (my.getType(obj) == 'Object' && Object.keys(obj).length == 0)
            || (my.getType(obj) == 'string' && ['{}', '[]'].includes(obj))
        ) {
            return true;
        } else {
            return false;
    ...
    backToDom:function(domEle) {
        $('html , body').animate({scrollTop: $(domEle).offset().top}, 800); // 滚动到元素dom 节点对象
    },
    window: {
        width: function() {
    ...
    toastr_init: function() {
        ...
        window.toastr.options = {
            ...
            "positionClass": "toast-top-center", //  居中 toast-center-center
            "onclick": null, //点击事件
                ...
                Object.keys(files).forEach(function(file) {
                    callback(files[file]).then(res => {
                        ...
                        if( res.is_done === true ){
                            ...
                            toastr.error(files[file].name + ':' + ( my.isEmpty( res['msg'] ) ? '上传失败': res['msg'] ),'上传异常提示')
                        }
                    }).catch(error => {
                        toastr.error(files[file].name + ' 上传失败','上传异常提示')
     ...
     */
     timestamp: function (dateString = '') {
        const time = dateString
            ? parseInt((new Date(dateString)).getTime().toString().substr(0, 10))
            : parseInt((new Date()).getTime().toString().substr(0, 10));
        return time;
    },
    /**
    ...
    urlToObject:function urlToObject(url) {
        ...
        try { urlArray = urlString.split("&"); } catch (error) { urlArray = new Array(url); }
