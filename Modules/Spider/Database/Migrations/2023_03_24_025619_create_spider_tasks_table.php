...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        Schema::create('spider_tasks', function (Blueprint $table) {
            ...
            $table->string('timer')->nullable()->comment('主任务爬取的时间周期,例如:1 0 * * *');
            ...
            $table->json('rules')->nullable()->comment('采集规则;格式["title"=>"xpath or css选择器","content"=>["need_clear"=>1,"route"=>["规则列表"]],"publish_time"=>["规则列表"],...];例:xpath(/html/body/div[6]/div[1]/div[2]/div[2])、元素名称(.content,#content)');
            ...
            $table->json('before_event')->nullable()->comment('采集前可触发的事件');
            $table->json('error_event')->nullable()->comment('采集出错时触发的事件');
            $table->json('finish_event')->nullable()->comment('采集完成后可触发的事件');
        ...
        \DB::statement("ALTER TABLE `spider_tasks` comment '爬虫采集任务'");
        ...
        \DB::statement("ALTER TABLE `spider_tasks_logs` comment '爬虫采集任务日志'");
        ...
        \DB::statement("ALTER TABLE `spider_links` comment '爬虫任务采集成功的 links; 用于去重，防止重复采集'");
    }
